...
class TaskManager {
  ...
  AddTask(task) {
    /* Task description */
    task.description = prompt("Enter your task here :");

    /* Create all task elements */
    let taskStructure = `
        <div id="desc_${task.id}">
            ${task.description}
        </div>
  ...
  CompletTask(button) {
    ...
    taskArray.forEach((task) => {
      if (task.id == x) {
        ...
        //description div
        let taskDesc = document.getElementById(`desc_${task.id}`)

        // disable buttons
        taskDiv.querySelectorAll('button')
          .forEach(button => button.disabled = true)
        // disable checkbox
        taskDiv.querySelectorAll('input')
          .forEach(button => button.disabled = true)
